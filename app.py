...
def venues():
    ...
    for venue in city_state:
        city = venue.city
        state = venue.state
        x = Venue.query.filter_by(city=city, state=state).all()
        venues = []
        for venue in x:
            id = venue.id
            name = venue.name
        ...
        y.append({"city": city, "state": state, "venues": venues})
    ...
    return render_template('pages/venues.html', areas=y)
...
def search_venues():
    ...
    Response = {"count": len(Venue_All), "data": y}

    # region Hard Code Response
    response = {
        "count": 1,
        "data": [{
            "id": 2,
...
def show_venue(venue_id):
    ...
    past_shows_count_query = ShowByVenue.filter(
        Show.start_time < datetime.now()).count()  # len(past_shows_query)
    past_shows_query = ShowByVenue.filter(
        Show.start_time < datetime.now()).all()
    upcoming_shows_count_query = ShowByVenue.filter(
        Show.start_time > datetime.now()).count()  # len(upcoming_shows_query)
    ...
    return render_template('pages/show_venue.html', venue=y)
...
def artists():
    ...
    return render_template('pages/artists.html', artists=y)
...
def search_artists():
    ...
    Artist_All = Artist.query.filter(
    ...
    Response = {"count": len(Artist_All), "data": y}
...
def show_artist(artist_id):
    ...
    return render_template('pages/show_artist.html', artist=y)
...
# endregion
    ...
    return render_template('pages/shows.html', shows=y)
...
def create_show_submission():
    ...
    if(Valid_Venue_Id==None and Valid_Artist_Id==None):
